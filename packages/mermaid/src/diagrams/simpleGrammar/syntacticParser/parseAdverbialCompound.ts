...
export function parseAdverbialCompound(node: GrammarNode): GraphicalNode {
  const validKeys: string[] = [adverbKey, adverbialKey, conjunctionFragmentKey, conjunctionKey];
  ...
  if (!allValid || node.children.length === 0) {
    throw new GrammarError('InvalidStructure', 'AdverbialCompound has unexpected structure');
  }
